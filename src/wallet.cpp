...
// etercoin: optional setting to unlock wallet for block minting only;
...
// etercoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // etercoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // etercoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // etercoin: not avatar mode
                    {
...
// etercoin: create coin stake transaction
...
// etercoin: check 'spent' consistency between wallet and txindex
// etercoin: fix wallet spent state according to txindex
...
// etercoin: disable transaction (only for coinstake)
